...
import {
  FiPackage, FiClock, FiCheck, FiX, FiPlay, FiRotateCw,
  FiTruck, FiFilter, FiChevronLeft, FiChevronRight
} from 'react-icons/fi'
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <div className="flex flex-wrap gap-3">
            {[
              { key: 'pending', label: 'Pending', icon: <FiClock />, color: 'text-yellow-700 bg-yellow-100 border-yellow-200' },
              { key: 'confirmed', label: 'Confirmed', icon: <FiCheck />, color: 'text-blue-700 bg-blue-100 border-blue-200' },
              { key: 'preparing', label: 'Preparing', icon: <FiRotateCw />, color: 'text-orange-700 bg-orange-100 border-orange-200' },
              { key: 'ready', label: 'Ready', icon: <FiPackage />, color: 'text-purple-700 bg-purple-100 border-purple-200' },
              { key: 'delivered', label: 'Delivered', icon: <FiTruck />, color: 'text-green-700 bg-green-100 border-green-200' },
              { key: 'cancelled', label: 'Cancelled', icon: <FiX />, color: 'text-red-700 bg-red-100 border-red-200' }
            ].map((f) => (
              <motion.button
                key={f.key}
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl border font-medium text-sm transition-all duration-200 ${
                  status === f.key
                    ? 'bg-gradient-to-r from-orange-500 to-amber-500 text-white shadow-md'
                    : `${f.color} hover:bg-white hover:shadow-sm`
                }`}
              >
                {f.icon}
                {f.label}
        ...
        {!loading && totalPages > 1 && (
          <div className="flex justify-center mt-8 gap-3">
            <button
              ...
              disabled={page === 1}
              className="p-3 bg-white rounded-2xl shadow-md border border-gray-200 hover:bg-gray-50 disabled:opacity-40"
            >
              <FiChevronLeft />
            </button>
            <span className="px-4 py-2 bg-white rounded-2xl border border-gray-200 font-semibold">
