...
export default function AccountEdit() {
  ...
  const onPhotoChange = (e) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      setMessage(t("Image too large (max 5MB)"));
      return;
    }
    if (!file.type.startsWith("image/")) {
      setMessage(t("Please select an image file"));
      return;
    }
    setPhotoFile(file);
  ...
  const onSave = async () => {
    ...
    if (!profile.firstName.trim()) {
      setMessage(t("First name is required"));
      setSaving(false);
      return;
    }

    if (!profile.lastName.trim()) {
      setMessage(t("Last name is required"));
      setSaving(false);
      return;
    }
    ...
    if (profile.phone.trim()) {
      ...
      if (!phoneRegex.test(profile.phone)) {
        setMessage(t("Please enter a valid phone number"));
        setSaving(false);
        return;
      }
    }
    ...
    try {
      ...
      if (photoFile) {
        ...
        if (resPhoto?.success && resPhoto.user?.profileImage) {
          setProfile((p) => ({
            ...p,
            profileImage: resPhoto.user.profileImage,
          }));
      ...
      setMessage(e.message || t("Failed to update"));
    }
    setSaving(false);
  };
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                <p className="mt-1 text-xs text-gray-500">
                  {t("Include country code if applicable")}
                </p>
              </div>
              ...
              <div className="md:col-span-2">
                ...
                <p className="mt-2 text-xs text-gray-500">
                  {t("Your address helps with accurate deliveries")}
                </p>
              </div>
            </div>
          </div>
